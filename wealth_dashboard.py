...
@media (max-width: 768px) {
    ...
    .ad-banner-frame img { max-width: 100%; height: auto; }
}
</style>
...
with tab2:
    ...
    with col_input:
        ...
        pension_val = st.number_input("受給年金額 (月額)", 0.00, 50.00, 15.00, step=0.01)
        ...
        if st.button("✨ 最短FIRE年齢を計算する", use_container_width=True):
            ...
            best_age = sim_rev.find_possible_fire_age({
                'currentAge': age, 'currentAssets': reg_assets + nisa_assets,
                'nisaAssets': nisa_assets, 'monthlyInvestment': monthly_inv,
                'expectedReturnPre': ret_pre, 'livingExpense': living_exp_monthly,
    ...
    with col_chart:
        ...
        res = simulator.calculate({
            'currentAge': age, 'currentAssets': reg_assets + nisa_assets,
            'nisaAssets': nisa_assets, 'monthlyInvestment': monthly_inv,
            'expectedReturnPre': ret_pre, 'fireAge': fire_age,
            'livingExpense': living_exp_monthly, 'expectedReturnPost': ret_post,
            'inflationRate': inf_rate, 'pensionAmount': pension_val,
        ...
        fig.update_layout(margin=dict(l=0, r=0, t=50, b=0), hovermode="x unified",
                          ...
                          yaxis=dict(rangemode="tozero")) # 縦軸を0から開始
